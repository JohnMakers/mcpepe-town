export const BUILDINGS = [
  {
    id: 'casino',
    name: 'Pepe Palace Casino',
    description: 'Spin the bag and try your luck at the slots.',
    x: 22, y: 8, w: 21, h: 31, // Top-left built into the hillside rock
    link: 'https://example.com/casino'
  },
  {
    id: 'arcade',
    name: 'McPepe Arcade',
    description: 'Level up and play the new meme classics.',
    x: 2, y: 30, w: 20, h: 32, // Mid-left
    link: 'https://example.com/arcade'
  },
  {
    id: 'gallery',
    name: 'McPepe Art Gallery',
    description: 'The latest digital meme masterpieces on display.',
    x: 17, y: 59, w: 22, h: 35, // Lower-left
    link: '/Gallery'
  },
  {
    id: 'trading',
    name: 'Stonks Exchange Trading Center',
    description: 'Dip buyers only. Trade your digital assets.',
    x: 77.5, y: 16, w: 21, h: 46, // Far top-right building with the chart screens
    link: 'https://example.com/exchange'
  },
  {
    id: 'cinema',
    name: 'McPepe Cinema',
    description: 'Now showing: Revenge of Pepe.',
    x: 60, y: 11, w: 20, h: 30, // Mid-right building (yellow/brown roof, "PEPE" on floor)
    link: '/cinema'
  },
  {
    id: 'whale',
    name: "McPepe Whale's Den",
    description: 'Private cave for meme whales only.',
    x: 60, y: 62, w: 33, h: 35, // Lower-right sea cave entrance
    link: 'https://example.com/whale'
  },
  {
    id: 'tower',
    name: "Pepe Tower",
    description: 'The central hub with digital screens.',
    x: 43, y: 14, w: 14, h: 60, // Center
    link: '#'
  }
];